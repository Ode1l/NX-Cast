...
bool playfair_copy_reply(uint8_t mode, uint8_t response[142])
{
    if (mode >= 4u || !response)
        return false;
    memcpy(response, g_reply_message[mode], sizeof(g_reply_message[mode]));
    return true;
}

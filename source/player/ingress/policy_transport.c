...
#define SOURCE_POLICY_LOCAL_PROXY_LOAD_OPTIONS \
    "hls-bitrate=min,cache-pause-initial=yes,cache-pause-wait=6,demuxer-seekable-cache=no,stream-lavf-o=seekable=0"
#define SOURCE_POLICY_HLS_GATEWAY_LOAD_OPTIONS \
    "hls-bitrate=min,cache-pause-initial=yes,cache-pause-wait=4,demuxer-seekable-cache=no"
...
void policy_apply_transport(PlayerMedia *media, const PlayerOpenContext *ctx)
{
    ...
    if (media->transport == PLAYER_MEDIA_TRANSPORT_HLS_LOCAL_PROXY)
    {
        ...
        media->demuxer_readahead_seconds = 4;
    ...
    if (media->transport == PLAYER_MEDIA_TRANSPORT_HLS_GATEWAY)
    {
        if (media->network_timeout_seconds < 18)
            media->network_timeout_seconds = 18;
        if (media->demuxer_readahead_seconds < 3)
            media->demuxer_readahead_seconds = 3;
        append_load_option(media->mpv_load_options,
                           sizeof(media->mpv_load_options),
                           SOURCE_POLICY_HLS_GATEWAY_LOAD_OPTIONS);

        if (media->format_hint[0] == '\0' ||
            strcmp(media->format_hint, ingress_format_name(media->format)) == 0 ||
            strcmp(media->format_hint, "hls-live") == 0 ||
            strcmp(media->format_hint, "hls-vod") == 0)
        {
            if (ingress_vendor_is_sensitive(media->vendor))
            {
                snprintf(media->format_hint,

...
static void soap_handler_on_player_event(const PlayerEvent *event, void *user)
{
    ...
    dlna_transport_sync_runtime_on_event(&g_soap_runtime_state, event);
    event_server_on_player_event(event);
}

static void sync_runtime_state_from_player_snapshot(void)
{
    dlna_transport_sync_runtime_from_snapshot(&g_soap_runtime_state);
}

static void reset_runtime_state_defaults(void)
{
